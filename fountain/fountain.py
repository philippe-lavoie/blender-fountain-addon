...
class Fountain:
    ...
    def parse(self):
        ...
        if ':' in contents[:20]:
            ...
            header = script_head.splitlines()
            self._parse_head(header)
            self._parse_body(script_body.splitlines(), len(header))
        else:
    ...
    def _parse_body(self, script_body, line_offset=0):
        ...
        for linenum, line in enumerate(script_body):
            ...
            if line[0] == '>':
                if len(full_strip) > 1 and line[-1] == '<':
                    if newlines_before == 0 and len(self.elements) > 0 and self.elements[-1].is_centered:
                        self.elements[-1].element_text = '\n'.join([self.elements[-1].element_text, full_strip[1:-1].strip()])
                    else:
